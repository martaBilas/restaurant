...
namespace restaurant.Server.Controllers
{
	...
	public class AdminController : ControllerBase
	{
		...
		public async Task<IActionResult> AddMeal([FromForm] AddMealCommand newMeal)
		{
			try
			{
				await _menuService.AddMealToMenu(newMeal.Name, newMeal.CategoryId, newMeal.Price, newMeal.Weight, newMeal.Image, newMeal.Description, newMeal.IsActive);
				return Ok();
			}
			catch (Exception ex)
			{
		...
		public async Task<IActionResult> UpdateMeal([FromForm] UpdateMealCommand newMeal)
		{
			try
			{
				await _menuService.UpdateMeal(newMeal.Id, newMeal.Name, newMeal.CategoryId ?? -1, newMeal.Price ?? -1, newMeal.Weight ?? -1, newMeal.Image, newMeal.Description, newMeal.IsActive);
				return Ok();
			}
			catch (Exception ex)
			{
