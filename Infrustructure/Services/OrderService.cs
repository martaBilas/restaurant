...
public class OrderService : IOrderService
{
    ...
    public List<OrderModel> GetOrdersList()
    {
       ...
       .Select(order => new OrderModel
       {
           ...
           Total = order.Total,
           OrderDate = order.OrderDate,
           OrderStatus = order.OrderStatusHistories
               .OrderByDescending(h => h.TimeOfChange)
               .Select(h => new OrderStatusModel
               {
                   Id = h.OrderStatus.Id,
    ...
    public async Task<bool> UpdateOrderStatusAsync(long orderId, long orderStatusId, long changedById)
    {
        var order = await _db.Orders
                          .Include(o => o.OrderStatusHistories)
                          .FirstOrDefaultAsync(o => o.Id == orderId);
        ...
        var orderStatus = await _db.OrderStatuses.FindAsync(orderStatusId);
        if (orderStatus == null) return false;

        var user = await _db.Users.FindAsync(changedById);
        ...
        order.OrderStatusHistories.Add(statusHistory);
